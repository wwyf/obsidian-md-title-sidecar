...
export class GraphInteractor {
    ...
    getGraph(): GraphView {
        //@ts-ignore
        if (this.plugin.app.internalPlugins.getPluginById('graph')) {
            // no need to bother if the graph is disabled
            return (
                this.getGraphOfType('graph') ??
                this.getGraphOfType('localgraph')
            );
        }
    }
}
